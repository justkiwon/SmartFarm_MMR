    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace_Goal')
            logger.debug(
...
class PickAndPlace_Goal(metaclass=Metaclass_PickAndPlace_Goal):
    ...
    _fields_and_field_types = {
        'pick_pose': 'sequence<double>',
        'place_pose': 'sequence<double>',
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.pick_pose = array.array('d', kwargs.get('pick_pose', []))
        self.place_pose = array.array('d', kwargs.get('place_pose', []))
    ...
    def pick_pose(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'pick_pose' array.array() must have the type code of 'd'"
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'pick_pose' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._pick_pose = array.array('d', value)
    ...
    def place_pose(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'place_pose' array.array() must have the type code of 'd'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'place_pose' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._place_pose = array.array('d', value)
...
class Metaclass_PickAndPlace_Result(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace_Result')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class Metaclass_PickAndPlace_Feedback(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace_Feedback')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class PickAndPlace_SendGoal_Request(metaclass=Metaclass_PickAndPlace_SendGoal_Request):
    ...
    _fields_and_field_types = {
        ...
        'goal': 'mmr_interfaces/PickAndPlace_Goal',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['unique_identifier_msgs', 'msg'], 'UUID'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['mmr_interfaces', 'action'], 'PickAndPlace_Goal'),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        from mmr_interfaces.action._pick_and_place import PickAndPlace_Goal
...
class Metaclass_PickAndPlace_SendGoal(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace_SendGoal')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from mmr_interfaces.action import _pick_and_place
...
class PickAndPlace_SendGoal(metaclass=Metaclass_PickAndPlace_SendGoal):
    from mmr_interfaces.action._pick_and_place import PickAndPlace_SendGoal_Request as Request
    from mmr_interfaces.action._pick_and_place import PickAndPlace_SendGoal_Response as Response
...
class PickAndPlace_GetResult_Response(metaclass=Metaclass_PickAndPlace_GetResult_Response):
    ...
    _fields_and_field_types = {
        'status': 'int8',
        'result': 'mmr_interfaces/PickAndPlace_Result',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.BasicType('int8'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['mmr_interfaces', 'action'], 'PickAndPlace_Result'),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        from mmr_interfaces.action._pick_and_place import PickAndPlace_Result
...
class Metaclass_PickAndPlace_GetResult(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace_GetResult')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class PickAndPlace_GetResult(metaclass=Metaclass_PickAndPlace_GetResult):
    from mmr_interfaces.action._pick_and_place import PickAndPlace_GetResult_Request as Request
    from mmr_interfaces.action._pick_and_place import PickAndPlace_GetResult_Response as Response
...
class Metaclass_PickAndPlace_FeedbackMessage(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace_FeedbackMessage')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class PickAndPlace_FeedbackMessage(metaclass=Metaclass_PickAndPlace_FeedbackMessage):
    ...
    _fields_and_field_types = {
        'goal_id': 'unique_identifier_msgs/UUID',
        'feedback': 'mmr_interfaces/PickAndPlace_Feedback',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['unique_identifier_msgs', 'msg'], 'UUID'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['mmr_interfaces', 'action'], 'PickAndPlace_Feedback'),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        from unique_identifier_msgs.msg import UUID
        self.goal_id = kwargs.get('goal_id', UUID())
        from mmr_interfaces.action._pick_and_place import PickAndPlace_Feedback
...
class Metaclass_PickAndPlace(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'mmr_interfaces.action.PickAndPlace')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class PickAndPlace(metaclass=Metaclass_PickAndPlace):
    ...
    from mmr_interfaces.action._pick_and_place import PickAndPlace_Goal as Goal
    # The result message defined in the action definition.
    from mmr_interfaces.action._pick_and_place import PickAndPlace_Result as Result
    # The feedback message defined in the action definition.
    from mmr_interfaces.action._pick_and_place import PickAndPlace_Feedback as Feedback

    class Impl:

        # The send_goal service using a wrapped version of the goal message as a request.
        from mmr_interfaces.action._pick_and_place import PickAndPlace_SendGoal as SendGoalService
        # The get_result service using a wrapped version of the result message as a response.
        from mmr_interfaces.action._pick_and_place import PickAndPlace_GetResult as GetResultService
        # The feedback message with generic fields which wraps the feedback message.
        from mmr_interfaces.action._pick_and_place import PickAndPlace_FeedbackMessage as FeedbackMessage
